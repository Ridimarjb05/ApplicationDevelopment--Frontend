...
export default function LoyaltyProgram() {
  ...
  async function handleSearch(e) {
    ...
    setLoading(true)
    try {
      const res = await getLoyaltySummary(customerId)
      setSummary(res.data)
    } catch (err) {
  ...
  async function handleCheckDiscount(e) {
    ...
    setDiscountResult(null)
    try {
      const res = await checkDiscountEligibility(customerId, subTotal)
    ...
    <div className="p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          View loyalty points and check discount eligibility
        </p>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-700 text-sm leading-relaxed">
              Customers automatically receive a <strong>10% discount</strong> on
              any single purchase where the subtotal exceeds{' '}
              <strong>Rs. 5,000</strong>. The discount is applied automatically
              when creating a sale invoice.
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-3 text-sm">
                    <div className="bg-white rounded-lg p-3 border border-green-100">
                      ...
                      <p className="font-bold text-slate-900">
                        Rs. {discountResult.subTotal?.toFixed(2)}
                      </p>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-green-100">
                      <p className="text-xs text-slate-500 mb-1">Discount (10%)</p>
                      <p className="font-bold text-green-600">
                        -Rs. {discountResult.discountAmount?.toFixed(2)}
                      </p>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-green-100">
                      <p className="text-xs text-slate-500 mb-1">Final Amount</p>
                      <p className="font-bold text-slate-900">
                        Rs. {discountResult.finalAmount?.toFixed(2)}
                      </p>
                    </div>
                  </div>
                )}
