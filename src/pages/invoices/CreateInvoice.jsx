...
export default function CreateInvoice() {
  ...
  useEffect(() => { api.get('/vendor?activeOnly=true').then(r => setVendors(r.data)).catch(() => {}) }, [])
  ...
  async function submit(e) {
    ...
    try {
      await api.post('/purchaseinvoice', { vendorID: Number(form.vendorID), partID: Number(form.partID), quantityPurchased: qty, unitCost: cost, notes: form.notes })
      navigate('/invoices')
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: '960px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <Link to="/invoices" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '13px', color: '#64748b', textDecoration: 'none', fontWeight: 500, marginBottom: '12px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '16px', alignItems: 'start' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e8ecf0', overflow: 'hidden', position: 'sticky', top: '20px' }}>
            ...
            <div style={{ padding: '0 16px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <Link to="/invoices" style={{ display: 'block', width: '100%', padding: '10px', borderRadius: '9px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontSize: '13px', fontWeight: 600, textDecoration: 'none', textAlign: 'center', boxSizing: 'border-box' }}>
                Cancel
              </Link>
            </div>
          </div>
